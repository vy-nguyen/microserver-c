# coding: utf-8

"""
    Simple API

    A simple API to demonstrate OpenAPI to C++ stub generation.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.list_common import ListCommon

class TestListCommon(unittest.TestCase):
    """ListCommon unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ListCommon:
        """Test ListCommon
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ListCommon`
        """
        model = ListCommon()
        if include_optional:
            return ListCommon(
                page_info = openapi_client.models.page_info.PageInfo(
                    total = 56, 
                    current = 56, 
                    seq_no = 56, 
                    item_count = 56, ),
                extras = [
                    openapi_client.models.item_id.ItemId(
                        uuid = '', 
                        kind = 'ucat', 
                        seq_no = 56, )
                    ]
            )
        else:
            return ListCommon(
                page_info = openapi_client.models.page_info.PageInfo(
                    total = 56, 
                    current = 56, 
                    seq_no = 56, 
                    item_count = 56, ),
        )
        """

    def testListCommon(self):
        """Test ListCommon"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
